    ...
    <div className="flex flex-col lg:flex-row h-full w-full overflow-hidden bg-background">
      <div className="flex-1 flex flex-col bg-card/40 min-w-0 overflow-y-auto">
        <div className="flex-1 p-3 sm:p-6">
          ...
          <div className="mb-4 sm:mb-6 rounded-xl border border-border bg-card/60 p-3 sm:p-4 backdrop-blur">
            ...
            <div className="flex flex-wrap gap-2">
              {song.sections.map((section) => (
                <div key={section.id} className="flex items-center gap-1 rounded-lg bg-background/50 p-1 border border-border/50">
                  <span className="px-1.5 text-[10px] sm:text-[11px] font-semibold text-muted-foreground uppercase">{section.name}</span>
          ...
          <div className="mb-6 sm:mb-8 p-3 sm:p-6 rounded-xl border border-border bg-card/80 backdrop-blur">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xs sm:text-sm font-semibold tracking-wider text-muted-foreground uppercase">Current Chord Voicing</h3>
              <div className="font-mono text-xl sm:text-2xl font-bold">{chord?.symbol || "-"}</div>
            </div>
